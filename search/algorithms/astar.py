...
class AStar(SearchAlgorithm):
    ...
    class AStarNode(Node, IntrusiveHeap.Node):
        """A search Node for running AStar Search Algorithm
    ...
    class Open(SearchAlgorithm.Open):
        ...
        def __init__(self):
            ...
            self.node_map: Dict[Space.State, AStar.AStarNode] = dict()

        def insert(self, node: Node):
            """Appends a Node into the Open list."""
            if not isinstance(node, AStar.AStarNode):
                raise TypeError("Only AStarNode is supported")

            self.node_map[node.state] = node
            self.heap.push(node)

        def pop(self) -> AStar.AStarNode:
        ...
        def sync_improvement(self, node: AStar.AStarNode):
    ...
    def reach(self, state: Space.State, action: Space.Action, parent: Node):
        ...
        if not isinstance(parent, AStar.AStarNode):
            raise TypeError("Only AStarNode is supported")
